...
export function chunkJavaFile(filePath: string, content: string, config: CtxConfig): CodeChunk[] {
  ...
  if (starts.length === 0) {
    return chunkTextFile(filePath, content, config, "java-block", undefined);
  }
  ...
  for (var s = 0; s < starts.length; s++) {
    ...
    var keywords = collect(part, /request\.getParameter\s*\(\s*["']([^"']+)["']\s*\)/g)
      ...
      .concat(collect(part, /session\.setAttribute\s*\(\s*["']([^"']+)["']\s*,/g))
      .concat(collect(part, /mapping\.findForward\s*\(\s*["']([^"']+)["']\s*\)/g))
      .concat(collect(part, /\b([A-Za-z_$][A-Za-z0-9_$]*Service)\s*\./g));
    var links = collect(part, /mapping\.findForward\s*\(\s*["']([^"']+)["']\s*\)/g);

    chunks.push(createChunk(filePath, chunks.length, typeName, name, start + 1, end, part, keywords, links, name ? [name] : []));
  }
  return chunks;
}
